...
defmodule Bytes.Rpc.LoggerMiddleware do
  ...
  def pre(%Context{meta: meta, headers: headers, body: body} = ctx) do
    trace_id = extract_or_generate_trace_id(headers)
    start_time = System.monotonic_time()
    %{service: service, cmd: cmd, node: node} = ctx.meta

    Task.start(fn ->
      Logger.info(
        "[RPC] Incoming #{node}.#{service}.#{cmd}.#{trace_id} -> headers=#{inspect(headers)}, request=#{inspect(body)}"
      )
    end)
  ...
  defp log_response(%Context{meta: %{service: service, cmd: cmd, node: node}} = ctx, data) do
    ...
    Task.start(fn ->
      Logger.info(
        "[RPC] Outgoing #{node}.#{service}.#{cmd}.#{trace_id} -> response=#{inspect(data)}, time=#{duration}μs"
      )
    end)
  end
